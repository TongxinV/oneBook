...
前面写的驱动我们都用`#define rGPJ0CON *((volatile unsigned int *)GPJ0CON)`的方式来访问寄存器，这样的做法在驱动中并不是很好。因为这样的做法在不同平台的情况下不具有可移植性。我们现在写的驱动是在ARM平台下去写的，ARM属于内存和IO统一编址的，我们在读写寄存器的时候即为进行IO操作，进行IO操作是和读写内存一样的（IO也有个地址），这就叫统一编址。但是还有另外一些CPU（像x86）是非统一编址的，这种CPU在进行IO操作时的方法跟进行内存的读写的方法是不一样的。那么在这种情况下就有一种问题，如果我们写的驱动不光要在ARM下运行，还要求在X86下也要能够运行，如果还用`#define rGPJ0CON *((volatile unsigned int *)GPJ0CON)`的方式肯定是不合适的，需要进行比较大的修改。我们要怎样才能够使他能够具有很强的移植性呢？——内核已经帮我们想好了办法，即内核提供访问寄存器的读写接口（函数），使用这些函数具有可移植性。其实现的原理就是用条件编译，如下比较：
