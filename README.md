...
- [ ] 使用系统的error来帮助调试
- [ ] http 长连接和短链接是什么；http怎么来实现长连接短链接；curl使用长连接的时候会出错问题
- [ ] 别人夸你的时候你要先跟别人说声谢谢
- [ ] 软件内部子模块的通信最好的应该是内存映射，可行吗？
- [ ] 想通过消息队列解耦和，实际上并却没有，因为rabbitmqRPC 模式的一些特性；所以没有什么是最好最优；即这是一个控制论的问题还是一个博弈论的问题
- [ ] 赵薇受访谈马云：马云1.没有觉得自己跟别人有什么不一样；2.身上有浓重的老师的气质，讲话中心思想明确，不会漫无边际没有重点，非常会提纲挈领。在那一段视频下，网友也提到了视频中的赵薇讲话逻辑性很强，词汇丰富，语言有魅力；看了这段评论我又学到了（源自于对自己的发问--我为什么没有发现网友说的）：从不同的立场看问题
- [ ] 框架设计：应用使用服务要不要对分层？
- [ ] 架构设计中的MQ思想和RPC思想
- [ ] SRT 中LOG功能
- [ ] 后端服务器开发需要开发哪些内容
- [ ] 多加log打印；多对个数进行限制；多做防御性检测
- [ ] 按时完成安排的工作是最低标准，知道是什么意思了吗？
- [ ] 考虑可能的异常情况
- [ ] 消息队列配合Session效果更好
- [ ] 两个函数的执行过程相似度很大的时候就要考虑抽象了--记对接项目
- [ ] 申请和释放在同一个接口做
- [ ] 在实现的时候就把接口封装好和写好，构思好【高内聚低耦合】才去实现代码
- [ ] **在编码的过程中，感觉还是不能Get到一些场景代码应该怎么实现比较好，后续要浏览相关代码学习总结**
<br>
